...
| Graceful non-WebGL fallback | **`/simulator` yes, `/pro-simulator` no.** **CORRECTION (2026-08-10):** the original audit recorded "none found" for both routes. That was wrong. `components/simulator/Simulator.tsx:15-19` has a real `hasWebGL()` capability check and a text fallback that describes the coverage path, the pattern and the m²/h figure. `ProTeardown.tsx` had none; `FLOORFORGE_07_a11y.patch` adds the same treatment. Neither route has a `webglcontextlost` handler. The false finding is left visible rather than quietly deleted. |
